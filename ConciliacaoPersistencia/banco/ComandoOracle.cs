...
namespace ConciliacaoPersistencia.banco
{
    /// <summary>
    /// Objeto que encapsula um dbCommand e facilita o uso atráves de métodos como Execute, AddParam, etc
    /// </summary>
    public class ComandoOracle : IDisposable
    {
        private OracleCommand cmd;
        private long lastInsertId;

        // "abre" um comando passando-se o sql, sem transação
        public ComandoOracle(ConexaoOracle conexao, string sql = "")
        {
            this.cmd = new OracleCommand(sql, conexao.GetConnection());
        }

        public ComandoOracle(OracleConnection conexao, string sql = "")
        {
            this.cmd = new OracleCommand(sql, conexao);
        }

        // com transação
     /*   public ComandoOracle(Transacao transacao, string sql = "")
        {
            if (!transacao.InTransaction)
                throw new Exception("Componente de transação não esta ativo para este comando");

            this.cmd = new OracleCommand(sql, transacao.GetTransaction().Connection, transacao.GetTransaction());
        }*/
        ...
        public DataTable GetDataTable(string sql = "")
        {
            ...
            using (OracleDataAdapter da = new OracleDataAdapter(this.cmd))
            {
        ...
        {
            this.cmd.Parameters.Add(parameter, value);
        }
        ...
        public OracleDataReader Select(string sql = "")
        {
